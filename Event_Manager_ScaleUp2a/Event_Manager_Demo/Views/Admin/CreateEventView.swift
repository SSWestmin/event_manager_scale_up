...
struct CreateEventView: View {
    ...
    @StateObject private var previewEventDataCoordinator: EventDataCoordinator
    @StateObject private var previewEventVM: EventViewModel
    ...
    init(adminVM: AdminViewModel) {
        ...
        let previewCoordinator = EventDataCoordinator()
        _previewEventDataCoordinator = StateObject(wrappedValue: previewCoordinator)
        _previewEventVM = StateObject(wrappedValue: EventViewModel(eventDataCoordinator: previewCoordinator))
    }

    private var draftEvent: EventModel {
        EventModel(
            user_id: adminVM.currentUserID ?? adminVM.user_id,
            eventName: adminVM.normalizedEventText(adminVM.eventName),
            eventDescription: adminVM.normalizedEventText(adminVM.eventDescription),
            eventStart: adminVM.eventStart,
            eventEnd: adminVM.eventEnd,
            eventLocation: adminVM.normalizedComposedLocation,
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    EventCard(event: draftEvent, eventVM: previewEventVM)
                        .environmentObject(previewEventDataCoordinator)
                }
                .padding(.horizontal)
                ...
                Button {
                    ...
                    // MARK: Success alert
                    guard adminVM.createAdminEvent(draftEvent, context: context) else {
